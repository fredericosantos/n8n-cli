from http import HTTPStatus
from typing import Any, cast
from urllib.parse import quote

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.get_executions_id_response_200 import GetExecutionsIdResponse200
from ...types import UNSET, Response, Unset


def _get_kwargs(
    id: float,
    *,
    include_data: bool | Unset = UNSET,
    redact_execution_data: bool | Unset = UNSET,
) -> dict[str, Any]:

    params: dict[str, Any] = {}

    params["includeData"] = include_data

    params["redactExecutionData"] = redact_execution_data

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/executions/{id}".format(
            id=quote(str(id), safe=""),
        ),
        "params": params,
    }

    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Any | GetExecutionsIdResponse200 | None:
    if response.status_code == 200:
        response_200 = GetExecutionsIdResponse200.from_dict(response.json())

        return response_200

    if response.status_code == 401:
        response_401 = cast(Any, None)
        return response_401

    if response.status_code == 403:
        response_403 = cast(Any, None)
        return response_403

    if response.status_code == 404:
        response_404 = cast(Any, None)
        return response_404

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | GetExecutionsIdResponse200]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    id: float,
    *,
    client: AuthenticatedClient | Client,
    include_data: bool | Unset = UNSET,
    redact_execution_data: bool | Unset = UNSET,
) -> Response[Any | GetExecutionsIdResponse200]:
    """Retrieve an execution

     Retrieve an execution from your instance.

    Args:
        id (float):
        include_data (bool | Unset):
        redact_execution_data (bool | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | GetExecutionsIdResponse200]
    """

    kwargs = _get_kwargs(
        id=id,
        include_data=include_data,
        redact_execution_data=redact_execution_data,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    id: float,
    *,
    client: AuthenticatedClient | Client,
    include_data: bool | Unset = UNSET,
    redact_execution_data: bool | Unset = UNSET,
) -> Any | GetExecutionsIdResponse200 | None:
    """Retrieve an execution

     Retrieve an execution from your instance.

    Args:
        id (float):
        include_data (bool | Unset):
        redact_execution_data (bool | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | GetExecutionsIdResponse200
    """

    return sync_detailed(
        id=id,
        client=client,
        include_data=include_data,
        redact_execution_data=redact_execution_data,
    ).parsed


async def asyncio_detailed(
    id: float,
    *,
    client: AuthenticatedClient | Client,
    include_data: bool | Unset = UNSET,
    redact_execution_data: bool | Unset = UNSET,
) -> Response[Any | GetExecutionsIdResponse200]:
    """Retrieve an execution

     Retrieve an execution from your instance.

    Args:
        id (float):
        include_data (bool | Unset):
        redact_execution_data (bool | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | GetExecutionsIdResponse200]
    """

    kwargs = _get_kwargs(
        id=id,
        include_data=include_data,
        redact_execution_data=redact_execution_data,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    id: float,
    *,
    client: AuthenticatedClient | Client,
    include_data: bool | Unset = UNSET,
    redact_execution_data: bool | Unset = UNSET,
) -> Any | GetExecutionsIdResponse200 | None:
    """Retrieve an execution

     Retrieve an execution from your instance.

    Args:
        id (float):
        include_data (bool | Unset):
        redact_execution_data (bool | Unset):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | GetExecutionsIdResponse200
    """

    return (
        await asyncio_detailed(
            id=id,
            client=client,
            include_data=include_data,
            redact_execution_data=redact_execution_data,
        )
    ).parsed
